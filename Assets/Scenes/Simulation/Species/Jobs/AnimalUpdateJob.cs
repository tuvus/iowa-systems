...
public struct AnimalUpdateJob : IJobParallelFor {
	...
	int eddibleCount;
	[ReadOnly] NativeArray<Eddible.Data> eddibles;
    ...
    public static JobHandle BeginJob(NativeArray<AnimalActions> animalActions, NativeArray<BasicAnimalScript.AnimalData> animals,int animalCount, float speciesFullFood, 
		...
		int eddibleCount, NativeArray<Eddible.Data> eddibles, int predatorCount, NativeArray<BasicAnimalScript.PredatorData> predators) {
		...
		AnimalUpdateJob job = new AnimalUpdateJob() {
			...
			availableFemaleMatesPositions = availableFemaleMatesPositions, eddibleCount = eddibleCount, eddibles = eddibles, predatorCount = predatorCount, predators = predators
		};
