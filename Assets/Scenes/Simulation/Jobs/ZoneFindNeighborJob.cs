...
public struct ZoneFindNeighborJob : IJobParallelFor {
    ...
    public static JobHandle BeginJob(NativeArray<float3> zonePositions, NativeMultiHashMap<int, int>.ParallelWriter neiboringZones, int maxNeiboringZones, float distance) {
        ZoneFindNeighborJob job = new ZoneFindNeighborJob() { zonePositions = zonePositions, neiboringZones = neiboringZones, maxNeiboringZones = maxNeiboringZones, distance = distance };
